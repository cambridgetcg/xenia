...
- Chronicle as append-only, attributed history: never overwrite what a past
  session recorded. Keep ordered bytes, actor claims, evidence, witnesses, and
  limitations so a current participant can inspect what the record says
  happened, was decided, or was committed. A chronicle can establish retention
  of those attributed bytes; it does not by itself prove the actor, connect the
  current caller to that actor, establish present consent, preserve earlier
  authority, or show that a bounded read was complete. Re-present historical
  commitments for inspection, never as automatically executable current
  authority.
